...
class Synsets
  ...
  def load(synset_file)
    ...
    f.each do |line|
      ...
      if (((line =~ /id:\s(\d+)\ssynset:\s([A-z]((,[A-z])?)+)+$/)  == nil) || (@synsets.has_key?($1.to_i)) || file_keys.include?($1.to_i))
        invalid_lines.push(line_num)
      else
        file_keys.push($1.to_i)
      end
    end
    f.close
...
class Hypernyms
  ...
  def load(hypernyms_file)
    ...
    f.each do |line|
      ...
      if (@hypernyms.has_key?($1.to_i))
        if !@graph.hasVertex?($1.to_i)
          @graph.addVertex($1.to_i)
        end
        arr = $2.split(',')
        arr.each do |noun|
          if (!(@hypernyms[$1.to_i].include?(noun.to_i)))
            @hypernyms[$1.to_i].push(noun.to_i)
            if !@graph.hasVertex?(noun.to_i)
              @graph.addVertex(noun.to_i)
            end
            @graph.addEdge($1.to_i, noun.to_i)
          end
        end
      else
        @hypernyms[$1.to_i] = []
  ...
  def parse(command)
    ...
    if (command =~ /\s*load\s*/)
      ...
      if ((command =~ /\s*lookup\s+(\d+)\s+(\S+)\s*/) != nil)
        ...
        h[:result] = @synsets.lookup($1.to_i)
      else
        h[:result] = :error
      end
      ...
      if ((command =~ /\s*lca\s+(\d+)\s+(\d+)\s+(\S+)\s*/) != nil)
        ...
        h[:result] = @hypernyms.lca($1.to_i, $2.to_i)
      else
        h[:result] = :error
      end
